...
| 作成日 | 2026-04-26 |
| 最終更新日 | 2026-05-07 |
| ステータス | Shopify正規方針・テーマ実装・SEO/メタデータ・ブランド名称反映済み |
| 位置付け | Shopify上でサイト・商品・ブログ・問い合わせ・決済運用を完結させる正規方針 |
| 参照元 | `web_mock/`、`docs/archive/オリジナル案/`、`work/Shopify案についての調査` |
...
| 領域 | 方針 |
|---|---|
| サイト名 / ブランド名 | 日本語表記は `四国ボタニカ` を正とする |
| 暫定英字表記 | イタリア語の botanica（植物・植物学）由来と推定し、テーマ・SEO・英字表示では `Shikoku Botanica` を暫定採用する。依頼者確認後に正式綴りへ差し替える |
| ブランド説明 | 四国の植物・薬草の知恵、イヌトウキ等の商品、喜多灘の古民家滞在を案内するボタニカルブランドとして表現する |
| プラットフォーム | Shopify Online Store |
| テーマ | Online Store 2.0 対応テーマをベースに、Liquid / JSON templates / sections / blocks でカスタマイズ |
| デザイン | `web_mock/` のトップ、商品、カート、古民家、問い合わせ、ブログの見た目を可能な範囲で再現。基準テーマカラーは暫定で `MOSS` |
| 商品管理 | Shopify Products / Collections / Variants / Inventory |
| 商品補足情報 | Product metafields / metaobjects |
| ブログ | Shopify Blogs / Blog posts |
| 古民家 | 初期実装はShopify Page + page metafields + page template。Metaobjectは写真・FAQ拡張時に追加 |
| 予約 | 独自予約システムは実装せず、外部予約URLへ誘導 |
| 問い合わせ | Shopify標準の `{% form 'contact' %}` を基本に、テーマ側で電話番号・問い合わせ種別・プライバシー同意を追加 |
...
| 問い合わせ | `{% form 'contact' %}` で問い合わせ送信を処理でき、モバイル操作手順も公式に案内されている | 標準送信機構 + テーマ側追加項目を採用 |
...
| コンテンツSEO | Shopifyブログは簡易で、本格メディアSEOには弱い | 初期はShopifyブログと商品・コレクションSEOを優先し、ヘッドレス構成は将来拡張として別途判断する |

本案を正規方針とし、EC運用・商品管理・決済をShopifyに集約する。ブランドストーリーやロングテール記事SEOを将来強化する場合は、Shopify内での改善を優先し、必要に応じてヘッドレス構成を新規拡張として検討する。

## 4. 対象ページ

| ページID | Shopify URL例 | ページ名 | 主データ | 優先度 |
|---|---|---|---|---|
| P01 | `/` | トップページ | Theme sections + Products + Blog posts | 必須 |
...
| P06 | `/pages/kominka` | 古民家紹介 | Page + page metafields + section settings | 必須 |
| P07 | `/pages/contact` | お問い合わせ | Shopify contact form + `main-page-contact` | 必須 |
...
- 注目商品はPC・スマホとも2列の画像中心表示とし、トップでは画像と商品名のみを表示する
...
- 商品一覧は画像、商品名、価格、NEWバッジを中心とし、数量指定とカート追加は商品詳細で提供すること
- 数量指定は在庫数と販売可否に応じて制御し、モックでは最大 9 個までの選択 UI として確認済み
...
- ヘッダーにはアイコンのみのカート導線を配置し、商品数バッジを表示する。件数表示は 99 点を超える場合 `99+` 表示を基本とする
...
- Shopify標準の contact form 送信機構を基本採用し、テーマ内の問い合わせsectionでフォーム表示を制御する
- 氏名、メールアドレス、電話番号、問い合わせ種別、本文、プライバシー同意を初期項目として運用する
- 問い合わせ種別はテーマエディタのブロックで変更できること
...
- ブログ表示名は `BLOG` に統一し、正規URLは `/blogs/stories` とする
- ブログ投稿にはタイトル、本文、アイキャッチ、抜粋、タグ、著者、SEOタイトル、SEOディスクリプションを設定できること
- 運営者アカウントでShopifyアプリから記事作成、画像追加、下書き保存、公開、編集ができることを実機確認する
- トップページに最新記事を表示する
- ブログ一覧は新しい記事を上にした縦並びのテキスト中心表示とし、画像は本文を邪魔しない小サムネイルにする
- ブログタグは枠線なしの文字リンクとし、`#` 付きの青系リンクとして表示する
- ブログ詳細のアイキャッチ画像は本文幅に合わせて過大表示を避ける
...
- 商品・コレクション・固定ページ・BLOG・メニューの初期投入はAdmin API seedスクリプトで補助できること
...
| 古民家情報 | Page + page metafields + section settings | 予約URL、写真、アクセス情報。Metaobjectは拡張時に利用 |
...
- 初期seedでは商品、コレクション、固定ページ、BLOG、記事のSEO title / meta descriptionを投入し、公開前に管理画面で確認・調整する
- テーマ側でOGP、Twitterカード、パンくず、主要構造化データの不足を補う
- 共通OGP画像は商品・記事画像、ShopifyのデフォルトOGP画像、テーマassetフォールバックの順で補完する
...
- 商品画像、コレクション画像、BLOGアイキャッチ画像のaltはseedまたは管理画面で空欄にしない
...
- Shopifyは基本SEOに強い一方、URL構造とブログ機能の自由度は低い。SEO強化が必要な場合は、Shopify内改善またはヘッドレス構成を将来拡張として検討する
...
| 2026-04-28 | 1.2.1 | Codex | `web_mock/` の商品画像遷移、数量指定、在庫表示、カート導線、アイコン件数バッジ、暫定テーマカラー `MOSS` を反映 |
| 2026-04-29 | 1.2.2 | Codex | 商品一覧の枠なしグリッド、トップ商品2列表示、NEWバッジ、BLOGスマホ更新、問い合わせ送信後フロー、CLI/seed運用を反映 |
| 2026-04-29 | 1.2.3 | Codex | 上位からの再帰レビューを実施し、問い合わせ追加項目、BLOG縦リスト、実装済みseed運用を要件へ反映 |
| 2026-04-30 | 1.2.4 | Codex | Shopify実装との差分を再帰確認し、初期SEOメタデータ、OGPフォールバック、画像alt、古民家page metafield運用を反映 |
| 2026-05-04 | 1.2.5 | Codex | Shopifyを正規方針とし、オリジナル案をアーカイブ参照へ変更 |
| 2026-05-07 | 1.2.6 | Codex | サイト名を四国ボタニカへ変更し、イタリア語 botanica 由来の植物・植物学ニュアンスをブランド説明と英字暫定表記へ反映 |
