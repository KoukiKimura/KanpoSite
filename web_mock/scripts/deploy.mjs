/**
 * web_mock FTP デプロイスクリプト
 * 接続情報は ../env/.env.mock.deploy から読み込みます
 */
...
// ─── env ファイルを読み込む ───────────────────────────────────────────────────
const envPath = resolve(__dirname, '../../env/.env.mock.deploy');
let envContent;
try {
  envContent = readFileSync(envPath, 'utf-8');
} catch {
  console.error(`[deploy] env ファイルが見つかりません: ${envPath}`);
  console.error('[deploy] env/.env.mock.deploy を作成してから再実行してください。');
  process.exit(1);
}
...
for (const key of required) {
  if (!env[key]) {
    console.error(`[deploy] 必須変数 ${key} が env/.env.mock.deploy に設定されていません。`);
    process.exit(1);
  }
}
